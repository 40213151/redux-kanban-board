...
export function InputForm({
  ...
  const ref = useAutoFitToVontentHeight(value)
 ...
 function useAutoFitToVontentHeight(content: string|undefined) {
  const ref = useRef<HTMLTextAreaElement>(null)

  useEffect(
    () => {
      // ref.currentはrefを適用している対象のDOMのこと
      const el = ref.current
      if (!el) return

      const { borderTopWidth, borderBottomWidth } = getComputedStyle(el)
      el.style.height = 'auto' // 一度autoにしないと高さが縮まらなくなる
